...
tgr_Short= r.TGraphErrors(len(Err_par1_Short),Detector_Short,np.absolute(par1_Short),errX_Short,Err_par1_Short)
tgr_Long= r.TGraphErrors(len(Err_par1_Long),Detector_Long,np.absolute(par1_Long),errX_Long,Err_par1_Long)	
...
tgr_Short.GetXaxis().SetTitleSize(0.05)
...
tgr_Short.GetXaxis().SetTitleOffset(1)
tgr_Short.GetXaxis().SetAxisColor(1)
tgr_Short.GetXaxis().SetTickLength(0.03)

#//------For Y axis---------//

tgr_Short.GetYaxis().SetTitle("LP Leak Parameter (from exp fit) [s^{-1}]")
tgr_Short.GetYaxis().SetNdivisions(508)
tgr_Short.GetYaxis().SetRangeUser(0.000001,0.01)
tgr_Short.GetYaxis().SetTitleSize(0.05)
...
tgr_Short.GetYaxis().SetTitleOffset(1.3)
...
fun_DrawLimit=r.TF1("DrawLimit","0.000062",0,5)
fun_DrawLimit.Draw("SAME")
fun_DrawLimit.SetLineStyle(5)

latex_Limit= r.TLatex(0.6,0.8,"")
latex_Limit.SetNDC()
latex_Limit.DrawLatex(0.36,0.50,"#color[2]{#font[12]{Acceptance Value#approx 6.2*10^{-5}}}")
latex_Limit.SetTextFont(4)
